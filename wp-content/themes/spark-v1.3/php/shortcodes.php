...
add_filter('the_content', 'spark_shortcode_formating_fix');
function spark_shortcode_formating_fix($content) {   
	$array = array (
		'<p>[' => '[', 
		']</p>' => ']', 
		']<br />' => ']',
...
// Columns
add_shortcode ('column', 'spark_column_shortcode');
function spark_column_shortcode($atts, $content = null){
	$atts['class'] = isset($atts['class']) ? $atts['class'] : '';
	return '<div class="'.$atts['width'].' columns '.$atts['class'].'">'.do_shortcode($content).'</div>';
}
...
function spark_tabs_container_shortcode($atts, $content){
	...
	$GLOBALS['spark_tabs_containers_count'] = (isset($GLOBALS['spark_tabs_containers_count'])) ? $GLOBALS['spark_tabs_containers_count'] + 1 : 0; // Allows to use the shortcode more than once on the same page.
	...
	if (is_array($GLOBALS['spark_tabs'])) {
		foreach($GLOBALS['spark_tabs'] as $tab){
			...
			$tabs[] = '<li><a class="'.$active_class.'" href="#tab-'.$GLOBALS['spark_tabs_containers_count'].$i.'">'.$tab['title'].'</a></li>';
			$panes[] = '<li class="'.$active_class.'" id="tab-'.$GLOBALS['spark_tabs_containers_count'].$i.'">'.do_shortcode($tab['content']).'</li>';
		}
...
function spark_pricing_column_shortcode($atts, $content = null){
	...
	if (!empty($atts['icon']) && substr($atts['icon'], 0, 7) == "http://") {
		$image_path = $atts['icon'];
	} else if (!empty($atts['icon'])) {
		$image_path = get_bloginfo('template_url').'/images/icons/'.$atts['icon'];
	} else {
		$image_path = get_bloginfo('template_url').'/images/icons/black/message_attention_icon&48.png';
	}
	
	ob_start();
	?>
		<div class="<?php echo (isset($atts['display_four_columns'])) ? 'four columns' : 'one-third column'; ?> <?php echo (isset($atts['featured'])) ? 'box light featured' : '' ?>">
...
// Adds an icon floating on the left of the content
...
function spark_icon_column_shortcode($atts, $content = null){
	...
	$atts['column'] = (isset($atts['column'])) ? $atts['column'] : '';
	...
	$return.= ((isset($atts['bigtitle'])) ? '<h3>'.$atts['title'].'</h3>' : '<h4>'.$atts['title'].'</h4>'); 
	...
	?>	
		<div class="row box <?php echo $color_class; ?>">
			...
			<div class="seven columns">
				...
				<?php if (!empty($spark_options['spark_main_button_text'])) : ?>
...
function spark_contact_form_shortcode() {
	...
	$return = <<<FORM
		...
		<form action="{$template_url_path}/php/others/contact-ajax.php" method="post" class="send-with-ajax">
